...
class TrueReqDiagram(AmbseBase):
    ...
    def log_presentation_elements(self):
        """
        Logs the PresentationElements of the diagram.
        """
        msg = "Current diagram: %s"%self.diagram.getDiagramType().getType()+"\n"
        msg += "PresentationElements: \n"

        for elem in self.elements:
            msg += self.elem2str(self.diagram.findPresentationElement(elem,None))
        self.log_message(msg)
